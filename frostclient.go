...
func GetObsMapForLabelApp(sourcesMap map[string]db.Camera) (map[int][]ObsRoadweather, error) {
...
func GetDataFromFrost3Classes(sourcesMap map[string]db.Camera) (map[int][]ObsRoadweather, error) {
	...
	for from.Before(stop) {
		...
		for t := 0; t < len(times); t++ {
			...
			*/
			...
			} else if roadConditionClass == SnowAndOrIce { // With or without water
				/*
...
func main() {
	...
	_, err = GetObsMapForLabelApp(sources)
	if err != nil {
